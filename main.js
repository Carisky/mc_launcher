...
const { createWindow, getMainWindow } = require('./main/windowManager');
...
const { getLatestUpdateInfo, downloadAndApplyUpdate } = createUpdateService({
  ...
  sendEvent: (payload) => {
    ...
    if (!window) return;
    try {
      window.webContents.send('app:update:event', payload);
    } catch (err) {
...
ipcMain.handle('mc:launch', async (event, payload = {}) => {
  ...
  let windowBeforeLaunch = getMainWindow();
  ...
  try {
    ...
    if (windowBeforeLaunch && !windowBeforeLaunch.isDestroyed()) {
      windowBeforeLaunch.hide();
